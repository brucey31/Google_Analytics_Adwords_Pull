...
def main(argv):
    ...
    try:
        ...
        if not first_profile_id:
            ...
            while page_iterator < end:
                results = get_top_keywords(service, first_profile_id, page_iterator)
                print_results(results, page_iterator)
...
def get_top_keywords(service, profile_id, page_iterator):
    ...
    return service.data().ga().get(
        ids='ga:' + profile_id,
        # start_date='%s' % datetime.date.today(),
        # end_date='%s' % datetime.date.today(),
        start_date='2015-01-01',
        end_date='2015-06-27',
        metrics='ga:uniqueEvents',
        dimensions='ga:date, ga:medium',
        filters='ga:eventAction==Completed').execute()


def print_results(results, page_iterator):
    ...
    # Print data table.
    if results.get('rows', []):
        # with open('Adwords_files/Adwords_Source_Data_%s.csv' % datetime.date.today(), 'wb') as csvfile:
        with open('Adwords_Source_Files/Adwords_Source_Data_2015_01_01.csv', 'wb') as csvfile:
...
#         print(',Adwords,'.join(output))
        ...
        # call(["s3cmd", "put", 'Adwords_Source_Files/Adwords_Data_%s.csv' % datetime.date.today(), "s3://bibusuu/Adwords_Data/"])
        call(["s3cmd", "put", 'Adwords_Source_Files/Adwords_Source_Data_2015_01_01.csv', "s3://bibusuu/Adwords_Data/"])
#
# # Delete local file
#         os.remove('Adwords_Source_Data.csv')
#
